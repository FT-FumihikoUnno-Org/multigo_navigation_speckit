...
- Completed: Development-only `dummyauth` OIDC provider, dev nginx reverse-proxy, local and CI Playwright E2E runner and tests, `approved` column migration and local-auth migrations and seed, local auth implementation (scrypt hashing + bcrypt fallback), `/auth/local` endpoints and tests, global Jest setup for backend and frontend, CI workflows that run unit tests (backend & frontend), and run E2E only after unit tests pass. / 完了: 開発専用 `dummyauth` OIDC プロバイダ、dev nginx リバースプロキシ、ローカルおよび CI 用 Playwright E2E ランナーとテスト、`approved` カラムのマイグレーション、ローカル認証用のマイグレーションとシード、ローカル認証の実装（scrypt ハッシュ + bcrypt フォールバック）、`/auth/local` エンドポイントとテスト、バックエンドとフロントエンドのグローバル Jest セットアップ、CI ワークフロー（ユニットテスト実行後に E2E を実行）。
- Verified: Backend and frontend unit tests and Playwright E2E smoke tests run locally; CI workflow updated to publish JUnit and Playwright reports. Admin CLI tests (unit + integration) and migration-based seeds were restored and validated locally; backend tests pass. / 検証済み: バックエンドとフロントエンドのユニットテストおよび Playwright E2E スモークテストはローカルで実行済み。CI ワークフローは JUnit と Playwright レポートの公開をサポートします。管理者 CLI のユニット/統合テストおよびマイグレーションによるシードは復元・検証済みで、バックエンドのテストはパスしています。
- Completed / Next: **Admin CLI implemented** and committed (`webapp/src/backend/src/cmd/bootstrap-admin.ts`, `webapp/src/backend/src/cmd/admin.ts`). The CLI is covered by unit and integration tests and is the recommended mechanism to bootstrap and manage admin accounts. Next: integrate any desired CI gating for the opt-in `bootstrap-admin` integration test and continue with frontend `force_password_change` UX integration and E2E stability monitoring. / 完了 / 次の対応: **管理者 CLI を実装してコミット済み**（`webapp/src/backend/src/cmd/bootstrap-admin.ts`, `webapp/src/backend/src/cmd/admin.ts`）。この CLI はユニット/統合テストでカバーされており、管理者アカウントのブートストラップと管理の推奨手段です。次は、オプトイン型の `bootstrap-admin` 統合テストを CI に組み込むか検討し、フロントエンドの `force_password_change` UX 統合と E2E 安定性監視を継続します。
...
- Backend: `Express.js`, `Passport.js` (for OpenID Connect), `pg` (for PostgreSQL)
...
**Constraints / 制約**: 
- The application must be offline-capable, allowing users to perform critical tasks even with a weak or no internet connection. / アプリケーションはオフライン対応でなければならず、インターネット接続が弱い、またはない場合でもユーザーが重要なタスクを実行できるようにする必要がある。
- The application must be accessible from mainland China, which prohibits the use of dependencies hosted on services like Google APIs (e.g., Google Fonts). / アプリケーションは中国本土からアクセス可能でなければならず、これによりGoogle APIなどのサービスでホストされている依存関係（例: Google Fonts）の使用が禁止される。
...
- **MUI (Material-UI)**: Provides a comprehensive set of React components that speeds up development. **For China accessibility, fonts (like Roboto) normally loaded from Google Fonts must be self-hosted within the application.** / **MUI (Material-UI)**: 開発を加速する包括的なReactコンポーネントセットを提供します。**中国でのアクセシビリティのため、通常Google Fontsから読み込まれるフォント（Robotoなど）は、アプリケーション内でセルフホストする必要があります。**
